function createtest(requestbody) { 
    ...
    try {
        ...
        var inserttoresult='INSERT INTO "WTH"."Questions" VALUES(?,?,?,?,?,?)';
        stmt = conn.prepareStatement(inserttoresult);
       
        stmt.setInteger(1,answers.id);
        stmt.setString(2,answers.IncidentID);
        stmt.setString(3,answers.User);
        stmt.setString(4,answers.Createdon);
        stmt.setString(5,answers.Question);
        stmt.setString(6,answers.Solution);
...
function getTests() { 
    ...
    try {
        conn = $.db.getConnection("wth.logic::dbuser");
        var sql = 'SELECT ID,User,CreatedOn,Question,Solution,IncidentID FROM "WTH"."Questions"';// WHERE QUESIONID=33';
...
try {
    ...
    } else if (method === $.net.http.OPTIONS) {
        $.response.status = 200;
        $.response.contentType = "application/json";
        $.response.setBody("");
       //$.response.setBody(JSON.stringify(createtest($.request.body.asString())));
